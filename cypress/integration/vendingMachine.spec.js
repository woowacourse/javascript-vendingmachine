...
describe('관리자 인증 UI 테스트', () => {
  ...
  it('[예외] 회원가입 화면에서 비밀번호, 비밀번호 확인값이 일치하지 않으면 스낵바로 메시지를 표시한다.', () => {
    cy.get('#signup-form__email').type('test@gmail.com');
    cy.get('#signup-form__name').type('민초');
    cy.get('#signup-form__password').type('1234567a');
    cy.get('#signup-form__password-confirm').type('1234567b');
  ...
  it('[예외] 회원가입 화면에서 비밀번호의 조건이 맞지 않으면 스낵바로 메시지를 표시한다.', () => {
    ...
    cy.get('#signup-form__email').type('test@gmail.com');
    cy.get('#signup-form__name').type('민초');
    cy.get('#signup-form__password').type('12345678');
    cy.get('#signup-form__password-confirm').type('12345678');
  ...
  it('회원가입 화면에서 입력 값의 조건이 맞으면 회원가입에 성공하고 로그인 페이지로 이동한다.', () => {
    ...
    cy.get('#signup-form__email').type(`${Math.random().toString(36).substring(8)}@gmail.com`);
    cy.get('#signup-form__name').type('민초');
    cy.get('#signup-form__password').type('1234567a');
    cy.get('#signup-form__password-confirm').type('1234567a');
  ...
  it('[예외] 로그인 화면에서 입력한 값이 서버에 존재하지 않는 경우 스낵바로 메시지로 표시한다.', () => {
    cy.get('#signin-form__email').type('notexist@notexist.com');
    cy.get('#signin-form__password').type('1234567a');
  ...
  it('로그인에 성공하면 자판기 관리화면으로 이동한다.', () => {
    ...
    cy.get('#signin-form__email').type('mincho@woowa.com');
    cy.get('#signin-form__password').type('1234567a');
...
describe('사용자 UI 테스트', () => {
  ...
  it('정상 금액을 투입하면 투입한 금액이 화면에 반영된다.', () => {
    cy.get('.purchase-form__money-input').type(CONFIGURATION.INPUT.MAX);
  ...
  it('[예외] 투입 금액 한도를 초과하면 메시지를 표시한다.', () => {
    ...
    cy.get('.purchase-form__money-input').type(CONFIGURATION.INPUT.MAX + 1);
    cy.get('.purchase-form__money-input-button').click();
