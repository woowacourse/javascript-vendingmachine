...
describe('회원가입 및 로그인 테스트', () => {
  ...
  it('2글자에서 6글자 사이가 아닌 이름으로 가입하려고 하면, 회원가입이 되지 않는다.', () => {
    ...
    cy.get('toast-modal').invoke('attr', 'message').should('eq', ERROR_MESSAGE.OUT_OF_NAME_LENGTH);
  });
  ...
  it('비밀번호 확인 칸에 다른 비밀번호를 입력하여 가입하려고 하면, 회원가입이 되지 않는다.', () => {
    ...
    cy.get('toast-modal').invoke('attr', 'message').should('eq', ERROR_MESSAGE.DIFFERENT_PASSWORD);
  });
  ...
  it('규칙에 어긋나는 비밀번호(7자리 이하)로 가입하려고 하면, 회원가입이 되지 않는다.', () => {
    ...
    cy.get('toast-modal').invoke('attr', 'message').should('eq', ERROR_MESSAGE.INVALID_PASSWORD);
  });
  ...
  it('적합한 회원 정보를 입력하면, 회원가입이 되어야 한다.', () => {
    ...
    cy.get('toast-modal').invoke('attr', 'message').should('eq', SUCCESS_MESSAGE.SIGNUP_COMPLETE);
  });

  it('가입된 정확한 이메일과 비밀번호를 입력하면, 로그인되어야 한다.', () => {
    cy.login(email, password);

    cy.get('toast-modal').invoke('attr', 'message').should('eq', SUCCESS_MESSAGE.LOGIN_COMPLETE);
  });
