...
describe('productManage', () => {
  const productManage = new ProductManageImpl([]);
  // 상품 추가
  test('상품이름이 1글자 미만일 때, 상품이 추가되지 않는지 확인', () => {
    const productInfo = {name: '', price: 1000, quantity: 10};
    expect(productManage.isValidProductInfo(productInfo)).toBe(false);
  });

  test('상품이름이 10글자 초과일 때, 상품이 추가되지 않는지 확인', () => {
    const productInfo = {name: '아상품명길게해야되는데뭘로하지모르겠네', price: 1000, quantity: 10};
    expect(productManage.isValidProductInfo(productInfo)).toBe(false);
  });
  ...
  test('상품정보가 올바르게 입력되었을 때, 상품이 추가되는지 확인', () => {
    ...
    expect(productManage.isValidProductInfo(productInfo)).toBe(true);
  });
  
  // 상품 수정
  test('상품이름이 1글자 미만일 때, 상품이 수정되지 않는지 확인', () => {
    const productInfo = {name: '', price: 4800, quantity: 10};
    expect(productManage.isValidModifyProductInfo(productInfo)).toBe(false);
  });

  test('상품이름이 10글자 초과일 때, 상품이 수정되지 않는지 확인', () => {
    const productInfo = {name: '자바칩 프라푸치노에 샷 추가에 휘핑크림 많이', price: 4800, quantity: 10};
    expect(productManage.isValidModifyProductInfo(productInfo)).toBe(false);
  });

  test('상품이름에 중복된 이름일 때, 상품이 수정되지 않는지 확인', () => {
    const productInfo = {name: '아메리카노', price: 4800, quantity: 10};
    expect(productManage.isValidModifyProductInfo(productInfo)).toBe(false);
  });
  ...
  test('상품정보가 올바르게 입력되었을 때, 상품이 수정되는지 확인', () => {
    ...
    expect(productManage.isValidModifyProductInfo(productInfo)).toBe(true);
  });
})