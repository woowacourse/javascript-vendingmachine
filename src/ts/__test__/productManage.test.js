...
describe('productManage', () => {
  ...
  test('상품이름이 1글자 미만일 때, 상품이 추가되지 않는지 확인', () => {
    ...
    expect(isValidProductInfo(productInfo, 0, productManage.products)).toBe(
      false,
    );
  });
  ...
  test('상품이름에 중복된 이름일 때, 상품이 추가되지 않는지 확인', () => {
    ...
    expect(isValidProductInfo(productInfo, 1, productManage.products)).toBe(
      false,
    );
  });
  ...
  test('상품이 올바르게 삭제되는지 확인', () => {
    ...
    expect(productManage.getProductIndex('아메리카노')).toBe(-1);
  });
});
