...
- [x] 관리자가 되고자 하는 유저는 회원 가입을 할 수 있다.
  - [x] 중복된 이메일 주소로는 가입할 수 없다.
  - [x] name은 2~6글자까지 가능하다.
  - [x] 비밀번호는 8~20자 길이여야 한다.
  - [x] 비밀번호는 영소문자, 숫자, 특수기호를 포함해야 한다.
- [x] 관리자는 로그인할 수 있다.
  - [x] 로그인하고 나면 로그인 버튼은 프로필 버튼으로 변경되어야 한다.
  - [x] 로그인한 유저만, 정보의 수정이 가능하다.
  - [x] 로그인하지 않은 유저에게는 로그인 외 다른 관리 기능 버튼은 보이지 않는다.
- [x] 로그인한 유저의 이름 중 첫번째 글자를 프로필 버튼처럼 만든다.
- [x] 로그인한 유저의 프로필 버튼을 클릭하면 select box로 회원 정보 수정과 로그아웃 메뉴가 표시된다.
- [x] 관리자는 로그아웃할 수 있다.
