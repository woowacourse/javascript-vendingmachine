...
- [x] 회원가입 후, 로그인할 수 있다.
  - [x] 로그인 버튼을 누르면 로그인 화면으로 바뀐다.
  - [x] 이메일과 비밀번호를 입력받는다.
  - [x] 일치하는 회원이 없으면 사용자에게 알린다.
  - [x] 로그인에 성공하면 상품 구매 화면으로 이동한다.
  - [x] 로그인한 유저만 탭 메뉴를 볼 수 있다.
- [x] 로그인한 유저는 우측 상단에 썸네일이 생긴다.
  - [x] 로그인한 유저의 이름 중 첫번째 글자를 썸네일로 만든다.
  - [x] 썸네일을 클릭하면 select box로 회원 정보 수정과 로그아웃 메뉴를 만든다.
  - [x] 회원 정보 수정을 누르면 아직 구현이 안 되었다는 snackbar를 띄운다.
- [x] 관리자는 로그아웃할 수 있다.
  - [x] 썸네일 클릭 후, 로그아웃 메뉴를 누르면 로그아웃 된다.
  - [x] 쿠키에 저장된 accessToken과 localStorate에 저장된 사용자 이름을 제거한다.
  - [x] 로그아웃하고 나면 썸네일이 로그인 버튼으로 변경된다.
  - [x] 로그아웃을 해도, 로그아웃 전에 설정한 상품 목록과 자판기 잔돈은 유지된다.
