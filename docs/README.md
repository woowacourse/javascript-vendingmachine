...
- 잔돈 반환
  ...
  - [ ] 반환 버튼이 클릭된다. (UI)
    - [ ] [예외] 투입한 금액이 10원 미만인 경우
    - [ ] [예외] 자판기가 보유한 동전이 부족한 경우
  - [ ] snackbar로 잔돈 반환 성공 여부를 보여준다. (UI)
  - [x] 투입한 금액을 줄인다. (도메인)
  - [x] 투입한 금액을 리렌더링 한다. (UI)
  - [x] 현재 보유한 최소 개수의 동전으로 잔돈을 돌려준다. (도메인)
  - [x] 잔돈 반환 테이블을 리렌더링 한다. (UI)
  - [x] 자판기가 보유한 동전을 리렌더링 한다. (UI)
  - [x] 현재 보유 금액을 줄인다. (도메인)
  - [x] 현재 보유 금액을 리렌더링 한다. (UI)
...
- 비로그인 사용자
  ...
  - [x] 로그인 버튼을 렌더링 한다. (UI)
  - [x] 상품 구매 - 투입할 금액을 입력하는 폼, 구매 가능 상품 현황, 잔돈 반환 테이블을 렌더링 한다. (UI)

  - 회원가입

    - [x] 로그인 버튼이 클릭된다. (UI)
    - [x] 로그인 폼을 렌더링 한다. (UI)
    - [x] 회원가입 버튼을 렌더링 한다. (UI)

    - [x] 회원가입 버튼이 클릭된다. (UI)
    - [x] 회원가입 폼을 렌더링 한다. (UI)
    - [ ] 이메일, 이름, 비밀번호, 비밀번호 확인이 입력된다. (UI)
      - [x] [예외 - 이메일] 이메일이 공백인 경우
      - [x] [예외 - 이메일] 이메일이 공백으로만 이루어진 경우
      - [x] [예외 - 이메일] 이메일이 이메일 형식이 아닌 경우
      - [ ] [예외 - 이메일] 이메일이 이미 존재하는 경우
      - [x] [예외 - 이름] 이름이 공백인 경우
      - [ ] [예외 - 이름] 이름이 공백으로만 이루어진 경우
      - [x] [예외 - 이름] 이름이 2글자 미만이거나 6글자를 초과하는 경우
      - [ ] [예외 - 비밀번호 확인] 비밀번호 확인이 비밀번호와 일치하지 않는 경우
    - [x] 확인 버튼이 클릭된다. (UI)
...
- 로그인 사용자
  ...
  - 초기 화면
    ...
    - [x] 썸네일을 렌더링 한다. (UI)
      - [x] 로그인한 유저의 이름 중 첫 번째 글자를 썸네일처럼 만든다.
    - [x] 관리자 메뉴를 렌더링 한다. (UI)
    - [x] 상품 구매 - 투입할 금액을 입력하는 폼, 구매 가능 상품 현황, 잔돈 반환 테이블을 렌더링 한다. (UI)
  ...
  - 회원 정보 수정
    ...
    - [x] 썸네일이 클릭된다. (UI)
    - [x] 회원 정보 수정과 로그아웃 메뉴를 렌더링 한다. (UI)
    - [x] 회원 정보 수정 메뉴가 클릭된다. (UI)
    - [x] 회원 정보 수정 폼을 렌더링 한다. (UI)
    - [x] 확인 버튼이 클릭된다. (UI)
