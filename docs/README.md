...
- [ ] 상품구매탭 HTML, CSS 마크업 작성

#### 1-1. 금액 투입

- [ ] 최초 투입된 금액은 0원이다.
- [ ] 투입 금액을 입력하고 투입 버튼을 누르면, 금액이 투입된다.
  - [ ] 투입 금액 유효성 검사
...
- [ ] 투입한 금액에 맞춰 재고가 있는 상품을 구매할 수 있다.
- [ ] 상품이 구매된 경우, 스낵바를 통해 알린다.
- [ ] 재고가 없는 상품에 대해 구매 버튼을 누른 경우, 재고없음 상태를 스낵바를 통해 알린다.

#### 1-3. 잔돈 반환

- [ ] 최초 반환된 각 동전의 개수는 0개이다.
- [ ] 반환하기 버튼을 누르면 다음 규칙에 의거 잔돈을 반환 받을 수 있다.

  - [ ] 잔돈을 돌려줄 때는 현재 보유한 최소 개수의 동전으로 잔돈을 돌려준다.
  - [ ] 잔돈을 1,000으로 나누었을 때 몫은 지폐로 반환되고, 나머지만 잔돈으로 반환한다.
  - [ ] 잔돈이 부족한 경우, 반환할 수 있는 금액만 반환한다.
  - [ ] 반환하지 못한 금액이 있는 경우, 스낵바를 통해 알린다.

### 2. 회원가입 탭

- [ ] 회원가입 탭 HTML, CSS 마크업 작성

- [ ] 관리자의 회원 가입을 위한 기능은 json-server-auth를 이용한다.
- [ ] 회원정보 입력 후 회원가입 버튼을 클릭하면, 회원가입이 완료된다.
  - [ ] 회원가입 유효성 검사
    - name은 2~6글자까지 가능하다.
    - password는 최소 10자리 이상이며 영어, 숫자, 특수문자 중 2종류 이상 조합이어야 한다.
    - 동일한 email로 중복 가입이 불가하다 .
  - [ ] 회원가입에 실패할 경우 스낵바를 통해 알린다.

### 3. 로그인 탭

- [ ] 로그인 탭 HTML, CSS 마크업 작성

- [ ] 아이디와 비밀번호를 입력하고 로그인 버튼을 클릭하면, 로그인이 완료된다.
  - [ ] 로그인 유효성 검사
    - 회원가입되어 있는 아이디이며 정확한 비밀번호임을 검증한다.
  - [ ] 로그인에 실패할 경우 스낵바를 통해 알린다.

### 4. 회원정보 수정 페이지

- [ ] 회원정보 수정 페이지 HTML, CSS 마크업 작성

- [ ] 이메일은 수정할 수 없다.(disabled)
- [ ] 이름 요소에 기존 이름이 적혀있다.
- [ ] 회원정보 수정 버튼을 누르면, 회원정보 수정이 완료된다.
  - [ ] 수정된 회원정보 유효성 검사
    - name은 2~6글자까지 가능하다.
    - password는 기존 password와 동일해야 한다.
  - [ ] 회원정보 수정에 실패할 경우 스낵바를 통해 알린다.

### 5. 비로그인 유저의 화면(상품 구매 페이지, 관리자 로그인 버튼)

- [ ] 비로그인 유저의 화면 HTML, CSS 마크업 수정

- [ ] 로그인하지 않은 유저에게는 로그인 외 다른 관리 기능 버튼은 보이지 않는다.

### 6. 로그인 유저의 화면

- [ ] 로그인 유저의 화면 HTML, CSS 마크업 수정

- [ ] 로그인하고 나면 로그인 버튼은 사라진다.
- [ ] 로그인한 유저의 이름 중 첫번째 글자를 썸네일처럼 만든다.
- [ ] 로그인한 유저의 썸네일을 클릭하면 select box로 `회원정보수정`과 `로그아웃` 메뉴가 표시된다.
- [ ] 로그아웃 버튼을 누르면, 비로그인 유저의 화면이 표시된다.
- [ ] 회원정보수정 버튼을 누르면, 회원정보수정 페이지가 표시된다.
